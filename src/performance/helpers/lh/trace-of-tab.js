...
class TraceOfTab {
  ...
  */
  static async compute_(trace) {
    ...
    if (!firstContentfulPaint) throw new Error('NO_FCP');
    ...
    if (!firstMeaningfulPaint) {
      ...
      fmpFellBack = true;
      console.log(`No firstMeaningfulPaint found, falling back to last ${fmpCand}`);
      const lastCandidate = frameEvents.filter(e => e.name === fmpCand).pop();
      if (!lastCandidate) {
        console.log('No `firstMeaningfulPaintCandidate` events found in trace');
      }
